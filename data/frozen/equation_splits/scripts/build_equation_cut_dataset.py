...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "experiments" / "2026-04-30_equation_cut_2026_pool_500_v0",
    )
    ...
    for tex_path in discover_tex_files(args.min_file_bytes):
        ...
        if normalize_paper_id(paper_id) in excluded_ids:
            scan_rows.append(
                {
                    "paper_id": paper_id,
                    "tex_file": str(tex_path.relative_to(ROOT)),
                    "status": "excluded",
        ...
        paper_summaries.append(
            {
                ...
                "title": item["title"],
                "tex_file": str(item["path"].relative_to(ROOT)),
            ...
            row.update(
                {
                    ...
                    "paper_source_path": str(item["path"].relative_to(ROOT)),
                    "paper_selection_rank": paper_rank,
...
def discover_tex_files(min_file_bytes: int) -> list[Path]:
    by_resolved: dict[str, Path] = {}
    for pattern in ("arxiv-26*.tex", "arXiv-26*.tex"):
        for path in TRAINING.glob(pattern):
...
def write_readme(out_dir: Path, manifest: dict) -> None:
    lines = [
        ...
        "- If an operator regex matches the prefix of a longer TeX command, the split is moved to after the full command, so `\\left` or `\\leftrightarrow` stays on the prefix side instead of producing half-command suffixes like `ft...`.",
