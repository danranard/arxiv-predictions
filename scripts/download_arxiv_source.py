...
def load_arxiv_ids_file(path: Path) -> list[str]:
    ...
    if lines and "," in lines[0] and ("paper_id" in lines[0] or "arxiv_id" in lines[0]):
        rows = csv.DictReader(lines)
        ids = []
        for row in rows:
            value = row.get("paper_id") or row.get("arxiv_id") or ""
            if value.strip():
                ids.append(normalize_arxiv_id(value.strip()))
    ...
    for line in lines:
        ...
        if not value or value.startswith("#"):
            continue
        ids.append(normalize_arxiv_id(value.split(",")[0]))
    return sorted(set(ids))
...
def main() -> int:
    ...
    parser.add_argument("--ids-file", type=Path, help="Optional text/CSV file of arXiv ids. CSV may contain paper_id.")
    ...
    if args.ids_file:
        raw_ids.extend(load_arxiv_ids_file(args.ids_file))
    if not raw_ids:
        raise SystemExit("Provide at least one arXiv id or --ids-file.")
